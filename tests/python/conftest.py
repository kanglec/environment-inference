...
def smoke_config_path(tmp_path: Path, project_root: Path) -> Path:
    ...
    text = text.replace('output_root = "../artifacts/local-smoke"', f'output_root = "{tmp_path / "out"}"')
    text = text.replace('scratch_root = "../scratch/local-smoke"', f'scratch_root = "{tmp_path / "scratch"}"')
